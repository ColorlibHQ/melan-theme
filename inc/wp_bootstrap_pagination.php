...
function melan_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'melan' ),
        'next_string'     => esc_html__( '&raquo;', 'melan' ),
        'before_output'   => '<div class="melan-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'melan_pagination_defaults', $defaults )
    );
