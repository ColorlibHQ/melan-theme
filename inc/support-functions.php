...
function melan_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'melan' ) . '</span></li>';
...
// Blog categories
function melan_featured_post_cat(){
...
// social media
if ( ! function_exists( 'melan_social' ) ) {
	function melan_social( $args = array() ) {
		...
		$url = melan_opt( 'melan_social_url' );
...
// header cart count
function melan_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( MELAN_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'melan' ); ?>">
...
function melan_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'melan_contact7_form_content', 10, 2 );