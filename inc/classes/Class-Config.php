...
final class Melan {
	...
	private function setup() {
		...
		$enqueu          = new melan_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->melan_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'melan_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'melan', MELAN_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'melan_site_logo_88x38', 88, 38, true );

		// Banner thumb sizes
		add_image_size( 'melan_banner_left_circle_724x767', 724, 767, true );
		add_image_size( 'melan_banner_right_circle_531x504', 531, 504, true );
		add_image_size( 'melan_banner_pattern_thumb_257x174', 257, 174, true );

		// Portfolio thumb sizes
		add_image_size( 'melan_portfolio_thumb_460x410', 460, 410, true );
		...
		add_image_size( 'melan_single_blog_750x375', 750, 375, true );
		add_image_size( 'melan_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' => esc_html__( 'Primary Menu', 'melan' ),
			'footer-menu'  => esc_html__( 'Footer Menu', 'melan' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = MELAN_DIR_CSS_URI;
		$jsPath  = MELAN_DIR_JS_URI;
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'melan' ) ) {
			
			$font_families = array(
				'Muli:300,300i,400,400i,600,600i,700,700i,800,800i,900',
			);
	...
	private function customizer_init(){
		...
		// Instantiate melan theme customizer
		$melan_theme_customizer = new melan_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'melan_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'melan-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'melan_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'melan-elementor-default-style',  MELAN_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->melan_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'melan_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'melan-elementor-notice', MELAN_DIR_JS_URI.'melan-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'melan-elementor-notice',
				'melanElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'melan-elementor-notice-nonce' ),
				)
			);
		}
